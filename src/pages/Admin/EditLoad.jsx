...
export default function AdminEditLoad() {
    ...
    <div className="w-full max-w-[1440px] mx-auto pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
        <div className="flex items-center gap-4">
          <button 
            onClick={() => navigate(`/admin/loads/${id}`)}
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-black text-gray-900 tracking-tight">Edit Load <span className="text-brand-orange">{id}</span></h1>
              <span className="bg-orange-50 text-brand-orange border border-orange-100 text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-widest">Modification Mode</span>
            </div>
            <p className="text-sm text-gray-500 mt-1 font-medium italic">Updating shipment manifest & administrative references</p>
          </div>
        </div>
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <button 
            onClick={handleSave}
            className="flex-1 md:flex-none px-10 py-3 bg-brand-yellow hover:bg-brand-orange text-black rounded-2xl font-black flex items-center justify-center gap-2 transition-all shadow-md active:scale-95"
          >
            <Save size={20} strokeWidth={2.5} /> Update Load
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex justify-between items-center px-2">
              ...
              <button 
                onClick={addItem}
                className="flex items-center gap-2 px-6 py-2.5 bg-gray-900 text-brand-yellow rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-black transition-all shadow-lg active:scale-95"
              >
              ...
              {items.map((item, index) => (
                <div key={item.id} className="bg-white rounded-3xl shadow-[0_10px_30px_-15px_rgba(0,0,0,0.1)] border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
                  <div className="p-5 border-b border-gray-100 bg-[#FAFAFA] flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="w-8 h-8 rounded-lg bg-gray-900 text-brand-yellow flex items-center justify-center font-black text-sm shadow-sm">{index + 1}</span>
                      <h3 className="text-xs font-black text-gray-900 uppercase tracking-widest">Item Specification</h3>
                    </div>
                    {items.length > 1 && (
                  ...
                  <div className="p-8">
                    ...
                    <div className="flex gap-4 mb-8">
                      {[
                        { id: 'vehicle', label: 'Car / Vehicle', icon: Truck, color: 'blue' },
                        { id: 'dangerous', label: 'Dangerous Goods', icon: AlertTriangle, color: 'orange' },
                        { id: 'general', label: 'General Freight', icon: Package, color: 'gray' },
                      ].map(type => (
                        <button
                          key={type.id}
                    ...
                    {item.niche === 'vehicle' ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-orange-50/30 p-6 rounded-2xl border border-orange-100/50">
                        ...
                        <div>
                          <label className="block text-xs font-bold text-orange-800 uppercase tracking-widest mb-2.5 ml-1">Hazard Class</label>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-6 rounded-2xl border border-gray-200/50">
                        <div className="md:col-span-2">
                          ...
                          <input type="text" value={item.description} onChange={e => updateItem(item.id, 'description', e.target.value)} placeholder="e.g. Pallet of Electronics" className="w-full bg-white border border-gray-200 focus:border-gray-500 rounded-xl py-3 px-4 text-sm font-bold text-gray-900 shadow-sm transition-all focus:outline-none focus:ring-4 focus:ring-gray-500/10" />
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-gray-600 uppercase tracking-widest mb-2.5 ml-1">Total Weight (kg)</label>
                          <input type="text" value={item.weight} onChange={e => updateItem(item.id, 'weight', e.target.value)} placeholder="e.g. 500" className="w-full bg-white border border-gray-200 focus:border-gray-500 rounded-xl py-3 px-4 text-sm font-bold text-gray-900 shadow-sm transition-all focus:outline-none focus:ring-4 focus:ring-gray-500/10" />
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-gray-600 uppercase tracking-widest mb-2.5 ml-1">Dimensions (LxWxH)</label>
                          <input type="text" value={item.dimensions} onChange={e => updateItem(item.id, 'dimensions', e.target.value)} placeholder="e.g. 1.2x1.2x1.5m" className="w-full bg-white border border-gray-200 focus:border-gray-500 rounded-xl py-3 px-4 text-sm font-bold text-gray-900 shadow-sm transition-all focus:outline-none focus:ring-4 focus:ring-gray-500/10" />
                        </div>
                      </div>
                    )}
                  </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-8">
          ...
          <div className="bg-[#111] rounded-[2.5rem] p-8 text-white shadow-2xl border border-gray-800 relative overflow-hidden group">
            <div className="absolute -right-12 -top-12 w-48 h-48 bg-brand-yellow/5 rounded-full w-fit blur-3xl group-hover:bg-brand-yellow/10 transition-all"></div>
            
            <h3 className="text-xs font-black uppercase tracking-[0.2em] mb-10 text-brand-yellow flex items-center gap-3 relative z-10">
              <Zap size={18} className="animate-pulse" /> Routing Configuration
            </h3>
            ...
            <div className="space-y-10 relative z-10">
              <div className="relative pl-8 border-l-2 border-dashed border-gray-800 pb-10">
                ...
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 px-4 text-sm font-bold text-white focus:outline-none focus:border-brand-yellow/50 transition-all" 
                />
              </div>

              <div className="relative pl-8 border-l-2 border-transparent">
                <div className="absolute -left-3 top-0 w-6 h-6 rounded-full w-fit bg-brand-yellow flex items-center justify-center shadow-[0_0_15px_rgba(255,191,0,0.3)]">
              ...
              <div className="pt-8 border-t border-gray-800 space-y-8">
                <div>
                  <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-4">Urgency Level</label>
                    ...
                    {['Low', 'Medium', 'High'].map(level => (
                      <button
                        ...
                        className={`flex-1 py-2 rounded-xl text-xs font-black transition-all uppercase tracking-widest ${
                          priority === level ? 'bg-brand-yellow text-black shadow-lg' : 'text-gray-400 hover:text-white'
                        }`}
                      >
                        {level}
                      </button>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-xl flex flex-col gap-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-4 border-b border-gray-50">
                <span className="text-xs font-bold text-gray-500">Total Manifest Items</span>
              ...
              <div className="flex justify-between items-center py-4 border-b border-gray-50">
                ...
                <span className="text-xs font-black px-3 py-1 bg-brand-yellow text-black rounded-lg uppercase tracking-widest">Premium Transit</span>
              </div>
            </div>
