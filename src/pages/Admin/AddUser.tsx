        ...
        <div className="md:col-span-1 flex flex-col gap-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="p-5 border-b border-gray-100 bg-gray-50/50 flex items-center gap-3">
              <UserCog className="text-brand-yellow" size={18} />
            ...
            <div className="p-6 flex flex-col gap-5">
              <div>
                ...
                <div className="relative group">
                  <UserCog className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-brand-yellow transition-colors" size={15}/>
                  <input type="text" placeholder="e.g. Liam Smith"
                    className="w-full bg-white border border-gray-200 focus:border-brand-yellow rounded-lg py-2.5 pl-11 pr-4 text-sm font-medium text-gray-900 shadow-sm transition-all focus:outline-none focus:ring-2 focus:ring-brand-yellow/20" />
                </div>
              </div>
              <div>
                <label className="block hero-metadata mb-1.5 ml-1">Work Email</label>
                <div className="relative group">
                  <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-brand-yellow transition-colors" size={15}/>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select value={selectedBranch} onChange={e => setSelectedBranch(e.target.value)}
                    className="w-full bg-white border border-gray-200 focus:border-brand-yellow rounded-lg py-2.5 pl-11 pr-10 text-sm font-medium text-gray-900 appearance-none focus:outline-none focus:ring-2 focus:ring-brand-yellow/20 transition-all">
          ...
          <div className="bg-[#111] rounded-xl p-5 text-white border border-gray-800 relative overflow-hidden">
            <div className="absolute -right-4 -top-4 w-24 h-24 bg-brand-yellow/10 rounded-full w-fit blur-2xl pointer-events-none"/>
            <p className="text-xs font-semibold text-brand-yellow uppercase tracking-widest mb-2 relative z-10">Selected Role</p>
                ...
                return (
                  <label key={r.id} onClick={() => setSelectedRole(r.id)}
                    className={`flex flex-col gap-3 p-5 border-2 rounded-xl cursor-pointer transition-all relative overflow-hidden ${
                      isSelected
                        ? r.dark ? 'border-[#111] bg-[#111]' : 'border-brand-yellow bg-[#FFFBEB]'
                    ...
                    <div className="flex justify-between items-start relative z-10">
                      ...
                      <div className={`w-5 h-5 rounded-full w-fit border-2 shrink-0 transition-all flex items-center justify-center ${
                        isSelected
                          ? r.dark
                        ...
                        {isSelected && <div className="w-2 h-2 rounded-full w-fit bg-[#111]"/>}
                      </div>
                    </div>
                    ...
                    <div className={`text-xs font-semibold uppercase tracking-widest  py-0.5 rounded w-fit relative z-10 ${
                      isSelected && r.dark ? 'bg-brand-yellow/20 text-brand-yellow' : 'bg-gray-100 text-gray-500'
                    }`}>
                      {r.access}
                    </div>
                  </label>
