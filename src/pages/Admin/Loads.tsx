...
export default function AdminLoads() {
  ...
  const rawJobs = [
    ...
    { id: 'SHP-9042', branchId: 'SYD-CENTRAL', customer: 'Acme Corp Logistics', customerRef: 'ACME-221', stockNumber: 'STK-4405', origin: 'Sydney Depot', dest: 'Melbourne Branch', queue: 'assigned', driver: 'Jack Taylor', vehicle: 'XQG-984', priority: 'High', eta: '14:30', pickup: '06:00 AM', window: 'Deliver by 16:00', load: '18.4t', notes: '', avatar: '/driver_avatar_2.png' },
    { id: 'SHP-9035', branchId: 'SYD-CENTRAL', customer: 'Southport Logistics', customerRef: 'SPL-55', stockNumber: 'STK-4406', origin: 'Adelaide Depot', dest: 'Sydney Depot', queue: 'assigned', driver: 'Oliver Brown', vehicle: 'V-102', priority: 'High', eta: 'Arrived at Branch', pickup: '05:00 AM', window: 'Delivered by 11:00', load: '12.0t', notes: '', avatar: '/driver_avatar_3.png' },
    { id: 'SHP-9041', branchId: 'SYD-CENTRAL', customer: 'Tech Solutions Ltd', customerRef: 'PO-8811', stockNumber: 'STK-4407', origin: 'Sydney Depot', dest: 'Penrith Branch', queue: 'exception', driver: 'Liam Smith', vehicle: 'BGT-221', priority: 'Medium', eta: 'Delayed', pickup: '07:00 AM', window: 'Deliver by 14:00', load: '9.5t', notes: 'Driver reports heavy traffic', exception: 'Delay', avatar: '/driver_avatar_4.png' },
    { id: 'SHP-9039', branchId: 'SYD-CENTRAL', customer: 'Global Traders AU', customerRef: 'GT-449', stockNumber: 'STK-4408', origin: 'Brisbane Depot', dest: 'Gold Coast Branch', queue: 'completed', driver: 'Liam Smith', vehicle: 'KLY-004', priority: 'Low', eta: 'Received', pickup: '03:00 AM', window: 'Deliver by 08:00', load: '5.5t', notes: '', avatar: '/driver_avatar_4.png' },
  ];
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:flex-initial">
              ...
              <input
                ...
                className="w-full md:w-64 bg-white border border-gray-200 hover:border-gray-300 rounded-lg py-2.5 pl-10 pr-4 text-sm font-normal text-gray-900 focus:outline-none focus:ring-2 focus:ring-brand-yellow/20 focus:border-brand-yellow transition-all shadow-sm"
              />
            </div>
            <button
          ...
          <div className="flex bg-gray-50 flex-wrap items-center gap-2 px-5 py-3 border-b border-gray-100">
            ...
            {['All', 'Local Pickups', 'Branch Transfers', 'Local Deliveries'].map(type => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full w-fit text-xs font-bold transition-all flex items-center gap-1.5 ${unassignedFilter === type ? 'bg-[#111] text-brand-yellow shadow-md' : 'bg-white border border-gray-200 text-gray-500 hover:bg-gray-100'
                  }`}
              >
                {type}
              </button>
        ...
        <div className="overflow-x-auto relative">
          ...
          {selectedIds.length > 0 && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 bg-hero-dark text-white px-6 py-4 rounded-hero-lg shadow-2xl flex items-center gap-6 animate-in slide-in-from-top-4 duration-300 border border-white/10">
              ...
              <button onClick={() => setSelectedIds([])} className="p-1 hover:bg-white/10 rounded-full w-fit transition-colors"><X size={16} /></button>
            </div>
          )}
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span className={`text-xs font-semibold uppercase tracking-widest px-3 py-1 rounded-md border shadow-sm transition-all ${
                      job.priority === 'High'  ? 'bg-[#111] text-brand-yellow border-brand-yellow/30 shadow-brand-yellow/10 shadow-lg' :
                      job.priority === 'Medium' ? 'bg-brand-yellow text-black border-transparent shadow-sm' :
