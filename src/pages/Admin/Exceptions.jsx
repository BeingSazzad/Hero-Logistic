  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto">
      <div className="flex justify-between items-end mb-2">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2"><AlertOctagon className="text-red-500"/> System Issues & Alerts</h1>
          <p className="text-sm text-gray-500 mt-1">High priority issues that need admin action to resolve.</p>
        </div>
      </div>
          ...
          <div key={idx} className="card bg-white shadow-sm border-l-4 border-l-red-500 p-0 overflow-hidden group">
            <div className="flex flex-col md:flex-row md:items-center">
               ...
               <div className="p-6 md:w-1/3 border-b md:border-b-0 md:border-r border-gray-100 bg-gray-50/50">
                 ...
                 <p className="text-xs font-bold text-gray-500 mt-1 uppercase tracking-widest">{alert.type} Issue</p>
               ...
               <div className="p-6 md:w-2/3 flex flex-col justify-between">
                 ...
                 <div className="mt-6 flex flex-wrap gap-3">
                    <button 
                      onClick={() => alert(`Overriding Security for ${alert.id}`)}
                      className="btn bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 text-xs py-2 px-4 flex items-center gap-2 font-bold shadow-sm"
                    >
                      <ShieldAlert size={14}/> Force Complete
                    </button>
                    <button 
                      onClick={() => alert(`Re-dispatching shipment ${alert.id}`)}
                      className="btn btn-dark text-xs py-2 px-4 flex items-center gap-2 font-bold focus:ring focus:ring-gray-300"
                    >
                      <RefreshCw size={14}/> Resend Driver
                    </button>
                    <button 
                      onClick={() => alert(`Shipment ${alert.id} is now on hold.`)}
                      className="btn bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 text-xs py-2 px-4 flex items-center gap-2 font-bold shadow-sm"
                    >
                      <Hand size={14}/> Stop Shipment
                    </button>
                 </div>
               </div>
