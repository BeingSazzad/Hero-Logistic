    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-3xl rounded-[2rem] shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-8 border-b border-gray-100 flex justify-between items-center shrink-0 bg-[#FAFAFA] rounded-t-[2rem]">
           <div>
              <h3 className="text-xl font-bold text-gray-900 tracking-tight flex items-center gap-3"><Car className="text-brand-yellow" /> Receive &amp; Slot Vehicles</h3>
        ...
        <div className="p-8 overflow-y-auto flex-1 flex flex-col gap-8">
           ...
           <div className={`border-2 rounded-3xl p-6 shadow-inner transition-all ${scanStatus.state === 'error' ? 'bg-red-50 border-red-200' : scanStatus.state === 'warning' ? 'bg-yellow-50 border-yellow-200' : scanStatus.state === 'success' ? 'bg-emerald-50 border-emerald-200' : 'bg-[#111] border-gray-800'}`}>
             <div className="flex justify-between items-center mb-3">
               ...
               <span className={`text-xs font-semibold uppercase tracking-widest ${scanStatus.state === 'error' ? 'text-red-400' : scanStatus.state === 'warning' ? 'text-brand-yellow' : scanStatus.state === 'success' ? 'text-emerald-400' : 'text-gray-600'}`}>
                  {scanStatus.msg || 'Awaiting Hardware Pulse...'}
               </span>
             </div>
             <div className="relative group">
               <Scan className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-brand-yellow transition-colors" size={20} />
               <input type="text" autoFocus value={scanInput} onChange={e => setScanInput(e.target.value)} onKeyDown={handleScan} placeholder="SCAN VIN BARCODE..." className="w-full bg-black/40 border border-white/5 rounded-2xl py-4.5 pl-14 pr-4 text-sm font-semibold text-brand-yellow focus:outline-none focus:ring-2 focus:ring-brand-yellow/50 shadow-2xl transition-all uppercase tracking-[0.1em]" />
             </div>
           </div>
        ...
        <div className="p-8 border-t border-gray-100 flex justify-between items-center bg-[#FAFAFA] rounded-b-[2rem] shrink-0">
           ...
           <div className="flex gap-4">
               <button 
                onClick={onConfirm} 
                disabled={!isComplete}
                className={`px-10 py-5 text-xs uppercase tracking-[0.15em] font-semibold rounded-2xl shadow-xl flex items-center gap-3 transition-all ${isComplete ? 'bg-gradient-to-r from-brand-yellow to-brand-orange hover:opacity-90 text-hero-dark active:scale-[0.98]' : 'bg-gray-100 text-gray-300 cursor-not-allowed'}`}
              >
    ...
    <div className="flex flex-col gap-6 w-full max-w-[1440px] mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4  mb-2">
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center justify-between group hover:shadow-xl transition-all">
          ...
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-gray-50 text-gray-400 group-hover:bg-brand-yellow group-hover:text-hero-dark transition-all"><Warehouse size={28}/></div>
        </div>
        <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center justify-between group hover:shadow-xl transition-all">
          <div>
                ...
                return (
                  <tr key={r.id} className={`transition-all group ${isReceived ? 'bg-emerald-50/10' : 'hover:bg-gray-50/50'}`}>
                    ...
                    <td className="px-6 py-6 text-center">
                      <div className="font-semibold text-gray-900 bg-gray-100 w-12 h-12 rounded-2xl flex items-center justify-center mx-auto text-lg shadow-inner group-hover:bg-brand-yellow transition-colors">{r.vehicles}</div>
                    </td>
                    <td className="px-6 py-6">
                    ...
                    <td className="px-6 py-6 text-right">
                      {!isReceived ? (
                        <button onClick={() => setActiveHandover(r)}
                          className="text-xs font-bold text-hero-dark bg-gradient-to-r from-brand-yellow to-brand-orange hover:opacity-90 px-6 py-3 rounded-xl transition-all uppercase tracking-[0.1em] flex items-center gap-2 shadow-lg ml-auto active:scale-95">
